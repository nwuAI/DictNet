...
from POSTER.poster_tool import PosterTool

os.environ['PATH'] += ':/home/pc/miniconda3/envs/deca/bin:/home/pc/miniconda3/condabin'
...
def main(args):
    ...
    savefolder = args.savefolder  # 设置保存文件夹以及运行设备
    device = args.device
    ...
    dict_exp, dict_pose = poster_tool.read_dictionary()
    # 依次处理每一张测试图像
    for i in tqdm(range(len(testdata))):
        name = testdata[i]['imagename']
        images = testdata[i]['image'].to(device)[None, ...]
        image_path = testdata[i]['image_path']
        exp_class = poster_tool.predict_image(image_path)
        with torch.no_grad():
            # 调用deca的编码器模块，获得低维潜向量，包括FLAME模型参数β，Ψ，θ，反照率系数α，相机c，和照明参数I。
            codedict = deca.encode(images)

            poster_exp = dict_exp[exp_class]
            poster_pose = dict_pose[exp_class]

            codedict['exp'] = poster_exp
            codedict['pose'][:, 3:6] = poster_pose[:, 3:6]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dictpath', type=str, default=r'./data/dictionary/exp2flame.pkl')
    parser.add_argument('--rootdir', type=str, default=r'./data/demo')
    parser.add_argument('--data', type=str, default=r'/home/Dataset/RAF')
    parser.add_argument('-e', '--evaluate', default='./checkpoint/raf-db-model_best.pth', type=str,
