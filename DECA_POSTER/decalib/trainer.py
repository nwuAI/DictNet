...
class Trainer(object):
    def __init__(self, model, exp_model, config=None, device='cuda:0'):
        if config is None:
            self.cfg = cfg
        else:
            self.cfg = config
        ...
        self.poster = exp_model
        self.configure_optimizers()
        self.load_checkpoint()

        # pc
        self.exp_prior = nn.Parameter(torch.tensor([0.5]), requires_grad=True)
    ...
    def training_step(self, batch, batch_nb, training_type='coarse'):
        self.deca.train()
        exp_dict = self.poster.read_dictionary()
        if self.train_detail:
        ...
        exp_class = self.poster.predict_images2(images)
